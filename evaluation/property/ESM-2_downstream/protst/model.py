...
class PretrainEvolutionaryScaleModeling(protst.ESM.ESM):
    """
    ...
    def __init__(self, path, model="ESM-1b", 
        ...
        #                               activation=self.activation)
        proj_path = ""
        self.seq_proj = torch.load(proj_path, map_location='cpu')
    ...
    def forward(self, graph, input, all_loss=None, metric=None):
        ...
        graph_feature = output["representations"][33][:, 0, :]
        if self.readout:
        ...
        if self.use_proj:
            ...
            graph_feature = self.seq_proj(graph_feature)
            residue_feature = self.seq_proj(residue_feature)
...
class PretrainProtBert(nn.Module, core.Configurable):
    ...
    def __init__(self, path, output_dim=512, num_mlp_layer=2, activation='relu', readout="pooler",
        ...
        #                               activation=self.activation)
        seq_proj = ""
        self.seq_proj = torch.load(os.path.join(base_path, seq_encoder_name) + '_seq_proj.pt', map_location='cpu')
    ...
    def forward(self, graph, input, all_loss=None, metric=None):
        ...
        output = self.model(input)
        residue_feature = output.last_hidden_state
        graph_feature = output.last_hidden_state[:, 0, :]
