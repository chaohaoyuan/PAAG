...
class PAAG_Pretrain(pl.LightningModule):
    def __init__(self,
        ...
        elif model_name_text == "pub_both":
            self.text_tokenizer = torch.load(os.path.join(input_path, "pubmedbert_both_tokenizer.pt"), map_location='cpu')
            self.text_encoder = torch.load(os.path.join(input_path, "pubmedbert_both.pt"), map_location='cpu')
        else:
...
class PAAG_Pretrain_Partial_Local(pl.LightningModule):
    def __init__(self,
        ...
        if model_name_text == "scibert":
            ...
            self.text_tokenizer = torch.load(os.path.join(input_path, "pubmedbert_both_tokenizer.pt"),
                                             map_location='cpu')
            self.text_encoder = torch.load(os.path.join(input_path, "pubmedbert_both.pt"), map_location='cpu')
        else:
            print("none text encoder found")
    ...
    def forward(self, seq, description, subsequences, noisy_subsequences, domains, alpha):
        ...
        output_pos = self.seq_encoder(encoder_input_ids,
                                      attention_mask=seq.attention_mask,
                                      encoder_hidden_states=text_output.last_hidden_state,
                                      encoder_attention_mask=text.attention_mask,  # might be wrong here
                                      return_dict=True,
                                      )
        with torch.no_grad():
            weights_s2t = F.softmax(sim_s2t[:, :bs], dim=1) + 1e-4
